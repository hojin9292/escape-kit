...
기다림 진열대 코너 [sealed shelf-corner, opensWhen: 4개 code:*-solved 전부]
  ─ 최종 콘솔 wait-shelf-order(서열 퍼즐, gate 없음) → door:time-open → 출구(엔딩)
```
...
| 최종 wait-shelf-order | TI-017·011·001·002 | 탭으로 순서 배치 | 정답 순열 유일 | `door:time-open`(게임 전체 출구) |
...
- 상황 카드 4개(세탁기 완료 후·부탁 후·친구 대답·엘리베이터)를 짧은 기다림부터
  긴 기다림 순서로 놓는다.
- 정답 순서(중앙값 오름차순, 초 단위): 세탁기(4.5) → 부탁(7) → 친구 대답(8) →
  엘리베이터(14).
