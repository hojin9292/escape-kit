...
## 2026-09-09 — 최종 퀴즈 난이도 완화 및 식사 에셋 정교화

- 여섯 방의 마지막 서열 퀴즈를 오답 없는 `복습 보드`로 교체했다. 앞에서 완료한
  네 활동을 어떤 순서로든 눌러 확인하면 통과하므로, 서로 다른 상황의 양·거리·시간을
  억지로 비교해야 했던 모호함을 없앴다.
- 복습 보드에는 활동명, 핵심 문장, 진행 도장(0/4)을 함께 표시하고 모바일에서는
  한 열로 배치한다.
- 잼 바르기는 식빵 실루엣과 얇은 잼 표현으로, 얼음 넣기는 컵 바닥의 한 겹 배치로
  조작 화면과 도구 에셋을 함께 수정했다.
- 여섯 최종 도구 에셋도 각 방에서 연습한 네 도구가 보이는 2×2 복습 카드로 교체했다.
