/**
 * 최종 콘솔: 힘 진열대 서열 퍼즐 (force-shelf-order). soap-shelf-order와 같은 구조.
 */
import "./puzzle.css";
...
export const forceShelfOrder: PuzzleModule = {
  ...
  mount(api: PuzzleApi): () => void {
    ...
    api.root.classList.add("ob-shelf-root");
    const sign = document.createElement("p");
    sign.className = "ob-shelf-sign";
    sign.textContent = "적은 힘부터 큰 힘 순서로 눌러서 진열대에 올려봐요.";

    const slots = document.createElement("div");
    slots.className = "ob-shelf-slots";
    slots.dataset.testid = "ob-shelf-slots";
    const slotEls = ITEMS.map((_, i) => {
      const el = document.createElement("div");
      el.className = "ob-shelf-slot";
      el.dataset.testid = `ob-shelf-slot-${i}`;
      const idx = document.createElement("span");
      idx.className = "ob-shelf-slot-index";
      idx.textContent = String(i + 1);
      const label = document.createElement("span");
      label.className = "ob-shelf-slot-label";
      label.textContent = i === 0 ? "가장 적게" : i === ITEMS.length - 1 ? "가장 크게" : "";
      el.append(idx, label);
    ...
    pool.className = "ob-shelf-pool";
    pool.dataset.testid = "ob-shelf-pool";

    const state = document.createElement("p");
    state.className = "ob-shelf-state";
    state.dataset.testid = "ob-shelf-state";
    state.textContent = "";

    const done = document.createElement("div");
    done.className = "ob-shelf-done";
    ...
    function renderSlots(): void {
      slotEls.forEach(({ el, label }, i) => {
        ...
        label.textContent = item ? item.name : i === 0 ? "가장 적게" : i === ITEMS.length - 1 ? "가장 크게" : "";
      });
    }
    function renderPool(): void {
      ...
      for (const item of remaining) {
        ...
        btn.type = "button";
        btn.className = "ob-shelf-card";
        btn.dataset.testid = `ob-shelf-card-${item.id}`;
    ...
    undoBtn.type = "button";
    undoBtn.className = "ob-shelf-undo-btn";
    undoBtn.dataset.testid = "ob-shelf-undo";
